...
const CategoryEnquiry = ({ show, category, page, pageSize, onHide }) => {
  ...
  const popupcategoryimage = useSelector(getAllCategorypopup);
  ...
  console.log("brandpopup", popupcategoryimage.result);
  const categoryselectedImage = useSelector(getCategorySelectedImage);
  console.log("selectedcategoryimage", categoryselectedImage);
    ...
    if ( product_name && email && phone_number && location) {
      try {
        ...
        if (res.status === 200) {
          ...
          // navigate("/about");
          dispatch(setCategoryShow(false));
  ...
  useEffect(() => {
    dispatch(fetchAsyncCategoryImages({category, page, pageSize}))
  }, [category, page, pageSize])
 
  let renderpopup = popupcategoryimage.result?.map((detail, index) =>(
    categoryselectedImage === index ? (
            <Modal size="lg" show={show}>
            <ModalHeader closeButton onClick={() => dispatch(setCategoryShow(false))}>
