...
// import { fetchAsyncCategoryImages, getAllCategorypopup, getCategorySelectedImage } from "../Redux/products/CategoryPopupSlice";
// import { setCategoryShow } from "../Redux/products/CategoryPopupSlice";
import { fetchAsyncSearchProducts, getAllSearchpopup, getSearchSelectedImage, setSearchShow } from "../Redux/products/SearchPopupSlice";
import {
  Button,
  Row,
  Col,
...
const SearchEnquiryForm = ({ show,debouceSearchTerm, onHide }) => {
  ...
  const popupsearchimage = useSelector(getAllSearchpopup);
  ...
  console.log(" beforesearchpopup", popupsearchimage);
  const filteredProducts = popupsearchimage.filter(item => 
    item.brand.toLowerCase().includes(debouceSearchTerm.toLowerCase()) ||
    item.product_name.toLowerCase().includes(debouceSearchTerm.toLowerCase()) ||
    item.category.toLowerCase().includes(debouceSearchTerm.toLowerCase()) ||
    item.subCategory.toLowerCase().includes(debouceSearchTerm.toLowerCase()) 
  );
  console.log(" after searchpopup", filteredProducts);
  const searchselectedImage = useSelector(getSearchSelectedImage);
  console.log("selectedsearchimage", searchselectedImage);
  ...
  useEffect(() => {
    dispatch(fetchAsyncSearchProducts(debouceSearchTerm))
  }, [debouceSearchTerm])
 
  let renderpopup = filteredProducts?.map((detail, index) =>(
    searchselectedImage === index ? (
            <Modal size="lg" show={show}>
            <ModalHeader closeButton onClick={() => dispatch(setSearchShow(false))}>
